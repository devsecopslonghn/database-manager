...
export async function evaluatePreflight(store: Store, plan: MigrationPlan): Promise<{ passed: boolean; checks: PreflightCheck[] }> {
  ...
  const checks: PreflightCheck[] = [
    { name: 'source_snapshot', status: plan.snapshotId || plan.items.every((item) => item.sourceType === 'MANUAL_UI') ? 'PASSED' : 'BLOCKED', detail: plan.snapshotId || plan.items.every((item) => item.sourceType === 'MANUAL_UI') ? undefined : 'PLAN_HAS_NO_SUCCESSFUL_SOURCE_SNAPSHOT' },
